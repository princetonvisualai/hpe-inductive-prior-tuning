...
from src.core.networks_2 import ParameterRegressor, Reconstructor
...
from src.core.utils.transforms import transform_template, transform_anchor_points, get_rotation_matrix
...
class Model:
    def __init__(self, cfg, device):
        ...
        self.aug = torch.eye(3).repeat(cfg['batch_size'],cfg['num_parts'],1,1)[:,:,2:,:].to(device)
    ...
    def train_step(self, frame1, frame2, return_imgs=False, predict=False):
        ...
        tot_parts_in_params = 20
        
        # Estimate the regressor parameters
        estimated_params = self.regressor(frame1) # shape: batch_size, 20 * 3 + 2

        scaling_matrix = estimated_params[:,-2:]
        transformation_bypart = estimated_params[:,:-2].view(-1, tot_parts_in_params, 3) # batch_size, 20, 3
        rotation_matrix = get_rotation_matrix(transformation_bypart[:,:,0])
        matrix = rotation_matrix * scaling_matrix[:,None,None]

        reposition = rotation_matrix * transformation_bypart[:,:,-2:][...,None,:]
        reposition_added = reposition[:,:,:,0] + reposition[:,:,:,1]
        matrix2_3 = torch.cat([matrix, reposition_added[...,None]], dim=-1)
        
        """
        matrix2_3 = torch.cat([matrix, transformation_bypart[:,:,-2:][...,None]], dim=-1)
        """
        ...
        estimated_params_1[:, 0:9] = matrix2_3[:, 0:9] # core, hips, thighs, shins, shoulders (9)
        estimated_params_1[:, [9,11,15]] = matrix2_3[:, 9].unsqueeze(1) # left arm, forearm, hand (3)
        estimated_params_1[:, [10,12,16]] = matrix2_3[:, 10].unsqueeze(1) # right arm, forearm, hand (3)
        estimated_params_1[:, 13:15] = matrix2_3[:, 11:13] # feet 2
        estimated_params_1[:, 17] = matrix2_3[:, 13] # head 1

        estimated_params_2[:, 9] = matrix2_3[:, 14] # left arm
        estimated_params_2[:, 11] = matrix2_3[:, 15] # left forearm
        estimated_params_2[:, 15] = matrix2_3[:, 16] # left hand
        estimated_params_2[:, 10] = matrix2_3[:, 17] # right arm
        estimated_params_2[:, 12] = matrix2_3[:, 18] # right forearm
        estimated_params_2[:, 16] = matrix2_3[:, 19] # right hand
        ...
        A_1 = torch.cat([estimated_params_1, self.aug], dim=-2) # batch_size, 18, 3, 3
        ...
        # Transform anchor points according to estimated parameters
        A_2 = torch.cat([estimated_params_2, self.aug], dim=-2) # batch_size, 18, 3, 3
