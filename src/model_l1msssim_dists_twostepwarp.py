...
class Model:
    ...
    def train_step(self, frame1, frame2, return_imgs=False, predict=False):
        ...
        frame1_inverse = self.INVERSE(frame1)[:,[2,1,0],:,:].cuda()
        reconstructed_inverse = self.INVERSE(reconstructed)[:,[2,1,0],:,:].cuda()
        reconstructed_inverse.requires_grad_()
        ms_ssim_loss = 1 - self.ms_ssim(frame1_inverse, reconstructed_inverse)
        dists_loss = self.D(frame1_inverse, reconstructed_inverse, require_grad=True, batch_average=True) 

        # overall loss, gradient and weight update
        loss = recon_l1_loss + ms_ssim_loss + dists_loss + self.lambda1 * anchor_loss + self.lambda2 * boundary_loss
        ...
        d = {'anchor_loss': anchor_loss, 'boundary_loss': boundary_loss, 
             'recon_l1_loss': recon_l1_loss, 'ms_ssim_loss': ms_ssim_loss, 'dists_loss': dists_loss}
        if return_imgs:
