...
NUM_TO_PICK = 5
if USE_TEACHER:
    TOT_SUPERVISION_EXAMPLES = 2 * (NUM_TO_PICK + len(cfg['supervision_idxs']))
else:
    TOT_SUPERVISION_EXAMPLES = 2 * NUM_TO_PICK
#####################################################
# define model
model = Model(cfg=cfg, device=device, num_examples_supervised = TOT_SUPERVISION_EXAMPLES, num_supervised_to_pick = NUM_TO_PICK)
...
def get_gts(indices):
    ...
    if USE_TEACHER:
        indices = indices + cfg['supervision_idxs']
...
for epoch in range(0, num_epochs):
    ...
    for batch in dataloader:
        ...
        elif epoch == MILESTONE and confused_indices is None:
            values, confused_indices = model.train_step(frame1, frame2, return_imgs, return_confused=True)
            supervision_idxs = idx[confused_indices].tolist()
            supervision_frame1, supervision_frame2, core_gt, double_gt, single_gt = get_gts(supervision_idxs)
        else: 
